...
def pyla_main(data, external_stop_event=None, external_pause_event=None):
    class Main:
        ...
        def recover_low_ips(self, current_ips):
            ...
            if self.low_ips_recovery_attempts >= self.low_ips_emulator_restart_after and hasattr(self.window_controller, "restart_emulator_profile"):
                try:
                    if not self.window_controller.restart_emulator_profile():
                        self.restart_brawl_stars()
                except Exception as exc:
