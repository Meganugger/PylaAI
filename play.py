...
class Movement:
    ...
    def __init__(self, window_controller):
        ...
        self._brawl_ball_lane_nudge_until = 0.0
        self._brawl_ball_lane_nudge_side = 1
    ...
    def must_brawler_hold_attack(cls, brawler, brawlers_info=None):
        ...
        brawler_key = cls._normalize_brawler_key(brawler)
        brawler_info = None
        for known, info in (brawlers_info or {}).items():
            if cls._normalize_brawler_key(known) == brawler_key:
                brawler_info = info
                break
        return bool((brawler_info or {}).get('hold_attack', 0) > 0)
    ...
    def _use_ability(self, ability_name, key, cooldown, last_attr, reason="ready_and_engaged"):
        current_time = time.time()
        ...
        self._record_battle_input(ability_name, reason)
        return True
    ...
    def use_hypercharge(self, reason="ready_and_engaged"):
        return self._use_ability(
            "hypercharge",
            "H",
            float(getattr(self, "hypercharge_cooldown", 2.5) or 0.0),
            "last_hypercharge_time",
            reason=reason,
        )
    ...
    def use_gadget(self, reason="ready_and_engaged"):
        return self._use_ability(
            "gadget",
            "G",
            float(getattr(self, "gadget_cooldown", 1.0) or 0.0),
            "last_gadget_time",
            reason=reason,
        )
    ...
    def use_super(self, reason="close_range_confirmed"):
        return self._use_ability(
            "super",
            "E",
            float(getattr(self, "super_cooldown", 1.0) or 0.0),
            "last_super_time",
            reason=reason,
        )
    ...
    def _update_target_gate(
        ...
        *,
        ...
        if self._is_verbose_battle_debug() and current_time - getattr(self, "_last_target_gate_log_at", 0.0) >= gate_interval:
            print(
                "[BATTLE] target gate "
    ...
    @staticmethod
    def should_use_super_on_enemy(brawler, super_type, enemy_distance, attack_range, super_range, enemy_hittable, close_range_contact=False):
        ...
        if super_type in {"spawnable", "other", "other_target"} and enemy_distance <= near_range:
            return True
        if super_type == "charge" and enemy_distance <= max(float(super_range or 0), float(attack_range or 0) * 1.25) and (enemy_hittable or brawler in {"stu", "surge"}):
            return True
        return False

    def try_use_super_on_enemy(self, brawler, brawler_info, player_pos, enemy_coords, enemy_distance, wall_context, close_range_contact=False):
        ...
        if not self.should_use_super_on_enemy(brawler, super_type, enemy_distance, attack_range, super_range, enemy_hittable, close_range_contact=close_range_contact):
            return False
        self.release_held_attack_for_super()
        if self.is_hypercharge_ready and self.use_hypercharge(reason="ready_and_engaged"):
            self.time_since_hypercharge_checked = time.time()
            self.clear_ability_ready("hypercharge")
            return True
        if self.use_super(reason="close_range_confirmed"):
...
class Play(Movement):
    ...
    def __init__(self, main_info_model, tile_detector_model, window_controller):
        ...
        self.hypercharge_cooldown = float(bot_config.get("hypercharge_cooldown", 2.5))
    ...
    def get_battle_runtime_state(self):
        return dict(getattr(self, "_battle_runtime", {}) or self._new_battle_runtime_state())

    def _battle_runtime_started(self):
        return float((self._battle_runtime or {}).get("match_started_at", 0.0) or 0.0) > 0.0
    ...
    def _note_battle_tick(self, current_time, brawler):
        ...
        if self._battle_runtime.get("active_brawler") != brawler_key:
            print(f"[MATCH] current brawler resolved: {brawler_key or 'unknown'}")
        if self._battle_runtime.get("active_logic_name") != logic_name:
            print(f"[BATTLE] selected logic: {logic_name}")
        self._battle_runtime.update({
            ...
            "active_logic_name": logic_name,
            "active_role": role,
            "fallback_active": logic_name == "GenericFallbackLogic" or bool(self._battle_runtime.get("fallback_active")),
        })
        if self._is_verbose_battle_debug() and current_time - self._last_battle_tick_log_at >= 2.0:
            strategy = self._battle_runtime.get("active_strategy") or "battle"
    ...
    def _record_battle_input(self, action, detail="", current_time=None):
        ...
        self._battle_runtime["last_input_at"] = current_time
        self._battle_runtime["last_action_at"] = current_time
        ...
        if self._is_verbose_battle_debug() and current_time - self._last_battle_input_log_at >= interval:
    ...
    def force_generic_fallback(self, reason, current_time=None):
        current_time = current_time if current_time is not None else time.time()
        self._battle_runtime["fallback_active"] = True
        self._battle_runtime["active_logic_name"] = "GenericFallbackLogic"
        self._battle_runtime["failure_count"] = int(self._battle_runtime.get("failure_count", 0) or 0) + 1
    ...
    def _estimate_player_detection(self, frame, data, current_time):
        ...
        if getattr(self, "_last_valid_player_bbox", None):
            ...
            if age <= 4.0:
                ...
                cx = width * 0.5
                cy = height * 0.58
        elif self._is_brawl_ball_mode(getattr(self, "selected_gamemode", "")):
            cx = width * 0.5
            cy = height * 0.70
            ...
            if self._uses_analog_movement():
                if self._is_brawl_ball_mode(self.selected_gamemode):
                    ...
                    if angle is None:
                        ...
                        self._plan_analog_reason(angle, "brawlball_opening")
                else:
                    angle = 270.0 if self.game_mode == 3 else 0.0
                if self._is_brawl_ball_mode(self.selected_gamemode):
                    angle = self._stabilize_analog_angle(angle, current_time)
    ...
    def load_brawler_ranges(self, brawlers_info=None):
        ...
        for brawler, info in brawlers_info.items():
            if not isinstance(info, dict):
                continue
            defaults = self._generic_brawler_info()
            attack_range = info.get('attack_range', defaults["attack_range"])
            safe_range = info.get('safe_range', defaults["safe_range"])
            super_range = info.get('super_range', defaults["super_range"])
    ...
    def _pick_showdown_orbit_side(self):
        ...
        if self._showdown_orbit_side not in (-1, 1):
            self._showdown_orbit_side = 1
        else:
            self._showdown_orbit_side *= -1
    ...
    def _get_showdown_follow_move(self, player_pos, wall_context, allow_spacing=True, allow_orbit=False):
        ...
        return teammate_move, teammate_distance, "team_follow"
    ...
    def _wall_blocked_escape_angle(self, angle, source, player_pos, wall_context, current_time=None):
        ...
        if bool(getattr(self, "is_showdown_mode", False)) and not is_brawl_ball:
            ...
            base_angle = self._find_best_angle(player_pos, lane_angle, wall_context)
            if self._angle_difference(base_angle, float(angle)) <= 8.0 and self._is_path_blocked_angle(player_pos, base_angle, wall_context):
                ...
                base_angle = self._find_best_angle(player_pos, base_angle, wall_context)
            nudge_angle = self._find_best_angle(player_pos, (base_angle + (34.0 * side)) % 360.0, wall_context)
    ...
    def _brawlball_no_vision_spawn_status(self, data=None, current_time=None):
        ...
        enemy_count = self._entity_count(data, "enemy")
        teammate_count = self._entity_count(data, "teammate")
        if enemy_count > 0 or teammate_count > 0:
            return None
    ...
    def _force_brawlball_no_vision_spawn_escape(self, data=None, current_time=None, source="spawn_escape_no_vision"):
        ...
        self._battle_runtime["last_attack_decision"] = "no-fire spawn escape"
        self._battle_runtime["last_movement"] = f"angle {angle:.0f}"
        print(
    ...
    def _get_brawl_ball_spawn_escape_angle(self, player_pos=None, current_time=None, candidate_angle=None, reason="inside_own_goal"):
        ...
        if candidate_angle is not None:
            ...
            if candidate_delta > 28.0:
                if current_time - getattr(self, "_last_brawl_ball_spawn_escape_log_at", 0.0) >= 0.8:
    ...
    def _get_brawl_ball_opening_angle(self, player_pos=None, current_time=None):
        ...
        if previous is not None and current_time < hold_until:
            delta = self._angle_difference(angle, previous)
            if delta > 35.0:
                if current_time - getattr(self, "_last_angle_smoothing_log_at", 0.0) >= 1.0:
                ...
                if current_time - getattr(self, "_last_angle_smoothing_log_at", 0.0) >= 1.0:
                    print(f"[BATTLE] movement stabilized previous={float(previous):.0f} new={float(angle):.0f}")
                    self._last_angle_smoothing_log_at = current_time
        else:
            self._brawl_ball_opening_angle_until = current_time + float(
                getattr(self, "_brawl_ball_opening_hold_seconds", 1.4) or 1.4
    ...
    def _get_corner_escape_angle(self, player_pos, current_time=None, no_target=True):
        ...
        width = max(1.0, float(getattr(self.window_controller, "width", brawl_stars_width) or brawl_stars_width))
        height = max(1.0, float(getattr(self.window_controller, "height", brawl_stars_height) or brawl_stars_height))
        target = (width * 0.50, height * (0.42 if self._is_brawl_ball_mode(getattr(self, "selected_gamemode", "")) else 0.50))
        angle = self.angle_from_direction(target[0] - player_pos[0], target[1] - player_pos[1])
    ...
    def _get_brawl_ball_roam_movement(self, player_pos, wall_context):
        ...
        if role in {"tank", "assassin", "fighter"}:
            center_target = (width * 0.50, height * 0.40)
            objective = "mid_pressure"
        elif role in {"sniper", "thrower"}:
            center_target = (width * 0.50, height * 0.46)
            objective = "lane_hold"
        else:
            center_target = (width * 0.50, height * 0.48)
            objective = "support_mid"
        now = time.time()
        if now - getattr(self, "_last_brawl_ball_objective_log_at", 0.0) >= 2.0:
            print(f"[BATTLE] brawlball objective=mid role={role} reason={objective}")
    ...
    def get_brawler_range(self, brawler):
        ...
        self._battle_runtime["fallback_active"] = True
        return [int(260 * scale), int(440 * scale), int(520 * scale)]
    ...
    def _set_battle_strategy(self, strategy):
        if not hasattr(self, "_battle_runtime") or self._battle_runtime is None:
            return
        self._battle_runtime["active_strategy"] = str(strategy or "battle")
    ...
    def get_movement(self, player_data, enemy_data, wall_context, brawler):
        ...
        if not brawler_info:
            ...
            self._battle_runtime["fallback_active"] = True
            self._set_battle_skip_reason(
                f"brawler info missing for '{brawler or 'unknown'}'; using generic fallback logic",
                fallback=True,
            )
        ...
        self._battle_runtime["active_role"] = self._playstyle_name(brawler)
        ...
        if not self.is_there_enemy(enemy_data):
            self._battle_runtime["last_attack_decision"] = "no target visible"
            if self._is_brawl_ball_mode(self.selected_gamemode):
                no_vision_data = {
                    "player": [player_data] if player_data is not None else [],
                    "enemy": [],
                    "teammate": self._teammate_positions,
                    "_player_source": getattr(self, "_last_live_player_source", "base"),
                }
                spawn_status = self._brawlball_no_vision_spawn_status(no_vision_data, current_time)
                if spawn_status:
                    self._battle_runtime["last_attack_decision"] = "spawn escape no vision; attack suppressed"
                    return self._plan_analog_reason(spawn_status["angle"], "spawn_escape_no_vision")
            if self.is_showdown_mode:
            ...
            if self._is_brawl_ball_mode(self.selected_gamemode):
                ...
                if search_move:
                    opening_elapsed = self._brawl_ball_opening_elapsed(current_time)
                    opening_active = opening_elapsed is not None and opening_elapsed <= self._brawl_ball_opening_seconds
                    self._set_battle_strategy("brawlball_opening" if opening_active else "brawlball_objective")
                    return search_move
                return self._get_brawl_ball_lane_push_angle(player_pos, current_time, wall_context)
        ...
        if enemy_coords is None:
            ...
            if self._is_brawl_ball_mode(self.selected_gamemode):
                self._set_battle_strategy("brawlball_objective")
                return self._get_brawl_ball_lane_push_angle(player_pos, current_time, wall_context)
            self._set_battle_strategy("fallback")
            return self.no_enemy_movement(player_data, wall_context)
        ...
        if self._is_brawl_ball_mode(self.selected_gamemode):
            ...
            if spawn_escape_angle is not None:
                high_confidence_spawn_threat = (
                    bool(target_hittable)
                    and enemy_distance <= min(220.0, effective_attack_range * 0.45, attack_range * 0.55)
                )
                if not high_confidence_spawn_threat:
                    self._log_brawl_ball_opening_override(False, "still_inside_spawn", current_time)
                    self._battle_runtime["last_attack_decision"] = "spawn escape locked; delaying target chase"
                    return self._plan_analog_reason(spawn_escape_angle, "brawlball_opening")
                self._log_brawl_ball_opening_override(True, "high_confidence_spawn_threat", current_time)
        if self._brawl_ball_opening_lock_active(current_time):
            high_confidence_override = (
                bool(target_hittable)
                and enemy_distance <= min(effective_attack_range * 0.55, attack_range * 0.65, 260.0)
            )
            if not high_confidence_override:
                self._log_brawl_ball_opening_override(False, "low_confidence/noisy", current_time)
                opening_angle = self._get_brawl_ball_opening_angle(player_pos, current_time)
                if opening_angle is not None:
                    self._battle_runtime["last_attack_decision"] = "opening route locked; delaying target chase"
                    return self._plan_analog_reason(opening_angle, "brawlball_opening")
            else:
                self._log_brawl_ball_opening_override(True, "close_hittable_enemy", current_time)
        ...
        if self._is_brawl_ball_mode(self.selected_gamemode):
            self._set_battle_strategy("brawlball_duel" if enemy_distance <= effective_attack_range else "brawlball_offense")
        elif should_retreat_for_ammo:
            self._set_battle_strategy("retreat")
        else:
            self._set_battle_strategy("duel")
        ...
        target_confidence = 1.0 if target_bbox is not None and (target_hittable or close_range_contact) else 0.72 if target_bbox is not None else 0.0
        confirmed_target = bool(enemy_data and enemy_coords is not None and target_bbox is not None)
        allow_attack_gate = (
            confirmed_target
            and enemy_distance <= effective_attack_range
            and (bool(target_hittable) or bool(style.get("fires_over_walls", False)) or close_range_contact)
        )
        allow_ability_gate = (
            confirmed_target
            and bool(target_hittable or close_range_contact)
            and enemy_distance <= max(super_range, attack_range)
        ...
        self._update_target_gate(
            ...
            target_age=0.0,
            target_source="real_detection",
            allow_attack=allow_attack_gate,
            allow_ability=allow_ability_gate,
            reason="fresh_target",
            current_time=current_time,
        )
        ...
        if self.is_showdown_mode:
            if showdown_fog_escape is not None:
                ...
                self._set_battle_strategy("showdown_survive")
                return self._plan_analog_reason(showdown_fog_escape, "fog_escape")

            if self._showdown_follow_enabled() and self._teammate_positions:
                teammate_target, teammate_distance = self._get_showdown_regroup_target(player_pos)
            ...
            if (
                ...
                if nearest_teammate is not None and teammate_distance > self._showdown_combat_regroup_distance:
                    ...
                    desired_angle = self.blend_angles(
                        desired_angle,
                        team_angle,
                        min(self._showdown_combat_regroup_bias, 0.18),
                    )
            movement = self._find_best_angle(player_pos, desired_angle, wall_context)
            ...
            self._set_battle_strategy(
                {
                    "retreat": "showdown_retreat",
                    "engage": "showdown_aggressive" if self._showdown_aggressive_enabled() else "showdown_duel",
                    "fog_escape": "showdown_survive",
                }.get(movement_reason, "showdown_duel")
            )
        ...
        if enemy_distance <= attack_range:
            if self.should_use_gadget == True and self.is_gadget_ready:
                if self.use_gadget(reason="enemy_in_range"):
                    self.time_since_gadget_checked = time.time()
                    self.is_gadget_ready = False
                    self._gadget_ready_seen_at = 0.0
            if self.is_hypercharge_ready and self.is_super_ready:
                if self.use_hypercharge(reason="ready_and_engaged"):
                    self.time_since_hypercharge_checked = time.time()
                    self.is_hypercharge_ready = False
                    self._hypercharge_ready_seen_at = 0.0
        if can_attack_now and (self.target_info["hittable"] or close_range_contact or style.get("fires_over_walls", False)):
            ...
            if attack_issued:
                self._battle_runtime["last_attack_decision"] = (
                    f"attack target distance={int(enemy_distance)} hittable={bool(self.target_info['hittable'])}"
                )
            ...
            self._battle_runtime["last_attack_decision"] = (
                f"hold fire target distance={int(enemy_distance)} hittable={bool(self.target_info['hittable'])}"
            )
    ...
    def main(self, frame, brawler):
        raw_brawler = brawler
        brawler = self.resolve_brawler_name(brawler)
        if raw_brawler and brawler and str(raw_brawler).strip().lower() != brawler:
            print(f"[MATCH] current brawler resolved: {brawler}")
        ...
        try:
            ...
            self._battle_runtime["failure_count"] = int(self._battle_runtime.get("failure_count", 0) or 0) + 1
            self._battle_runtime["fallback_active"] = True
            if current_time - self._last_battle_error_log_at >= 1.0:
                print(f"[BATTLE][ERROR] battle loop failed for {brawler or 'unknown'}: {exc}")
