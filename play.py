...
from utils import load_toml_as_dict, count_hsv_pixels, load_brawlers_info, reader as shared_reader
...
class Movement:
    ...
    def __init__(self, window_controller):
        ...
        self._brawl_ball_opening_lock_seconds = float(bot_config.get("brawl_ball_opening_lock_seconds", 4.5))
        self._brawl_ball_opening_hold_seconds = float(bot_config.get("brawl_ball_opening_hold_seconds", 1.4))
        self._brawl_ball_spawn_escape_seconds = float(bot_config.get("brawl_ball_spawn_escape_seconds", 8.0))
        self._brawl_ball_spawn_escape_min_seconds = float(bot_config.get("brawl_ball_spawn_escape_min_seconds", 5.8))
        self._brawl_ball_spawn_escape_nudge_after = float(bot_config.get("brawl_ball_spawn_escape_nudge_after", 6.2))
        self._brawl_ball_spawn_escape_nudge_interval = float(bot_config.get("brawl_ball_spawn_escape_nudge_interval", 1.8))
        ...
        self._brawl_ball_spawn_escape_complete = False
        self._brawl_ball_spawn_escape_started_at = 0.0
        self._brawl_ball_spawn_escape_last_nudge_at = 0.0
        self._last_brawl_ball_spawn_escape_log_at = 0.0
        ...
        self._analog_goal_hold_times = {
            ...
            "team_follow": float(bot_config.get("analog_team_follow_hold_time", bot_config.get("analog_team_hold_time", 0.28))),
        ...
        self._brawl_ball_spawn_escape_uncertain_seconds = float(bot_config.get("brawl_ball_spawn_escape_uncertain_seconds", 10.0))
        self._brawl_ball_spawn_escape_extended_seconds = float(bot_config.get("brawl_ball_spawn_escape_extended_seconds", 14.0))
        self._corner_escape_active = False
        self._corner_escape_until = 0.0
        self._last_corner_escape_log_at = 0.0
        self._authoritative_movement_angle = None
        self._authoritative_movement_source = ""
        self._authoritative_movement_at = 0.0
        self._wall_blocked_threshold_seconds = float(bot_config.get("wall_blocked_escape_seconds", 0.7))
        ...
        self.HYPERCHARGE_USE_COOLDOWN = float(bot_config.get("hypercharge_cooldown", 2.5))   # seconds between hypercharge uses
        self.hypercharge_cooldown = self.HYPERCHARGE_USE_COOLDOWN
        self.last_hypercharge_time = 0.0
        ...
        self._enemy_accel = (0.0, 0.0)        # Estimated acceleration (px/secÂ²)
    ...
    def estimate_hp_from_bar(frame, bbox, is_player=False, debug_info=None):
        ...
        try:
            ...
            # Uses numpy diff-based segmentation - ~10Ã— faster than Python loops.
            ...
            # Strategy: look at full-bar rows near the best health row (Â±3 rows)
    ...
    @staticmethod
    def _role_desired_range_label(role):
        role = str(role or "fighter").lower()
        if role == "tank":
            return "close"
        if role == "assassin":
            return "close_flank"
    ...
    def _get_lead_offset(self, distance, playstyle):
        ...
        # s = v*t + 0.5*a*tÂ²  (predict with acceleration for curving movement)
    ...
    def _get_intercept_position(self, player_pos, enemy_pos, enemy_distance):
        ...
        pred_y = enemy_pos[1] + vy * time_to_reach * 0.6  # vy includes 1.25Ã— isometric
    ...
    def _detect_poison_gas(self, frame):
        ...
        try:
            ...
            # Divide frame into a 3Ã—3 grid and measure gas density each region
            rh, rw = fh // 3, fw // 3
            densities = {}
            for gy in range(3):
                for gx in range(3):
    ...
    def _trigger_reactive_dodge(self, player_pos, attacker_pos):
        ...
        # Perpendicular: rotate 90Â° (randomly left or right)
    ...
    def find_best_target(self, enemy_data, player_coords, walls, skill_type, attack_range):
        ...
        for enemy in enemy_data:
            ...
            if hittable:
                ...
                # Throwers can still lob over walls â€” treat as a partial target.
                score += 60
    ...
    def read_enemy_hp_number_fast(frame, bbox, _ocr_cache={}):
        ...
        try:
            ...
            # Reuse the shared OCR reader so the branch follows the same CPU/GPU safety logic.
            if 'reader' not in _ocr_cache:
                _ocr_cache['reader'] = shared_reader
    ...
    def angle_opposite(angle_degrees):
        return (float(angle_degrees) + 180.0) % 360.0
    ...
    def _is_showdown_behavior_enabled(cls, gamemode, gamemode_type):
        ...
        try:
            mode_type = int(gamemode_type or 0)
        except (TypeError, ValueError):
            mode_type = 0
        return "showdown" in normalized and mode_type == 3
    ...
    def attack(self, touch_up=True, touch_down=True):
        if not self._can_issue_live_input():
            return False
        if touch_up and touch_down and self._action_used_this_tick("attack"):
            if debug:
                print("[BATTLE] attack skipped reason=action_budget")
            return False
        if touch_down and not self._target_gate_allows("attack"):
            self._log_target_gate_skip(
                "attack",
                str(getattr(self, "_last_target_gate_block_reason", "") or "stale_or_missing_target"),
            )
            return False
    ...
    def _action_used_this_tick(self, action):
        ...
        if not budget.get("tick_started_at"):
            return False
        return bool(budget.get(f"{action}_sent"))

    def _mark_action_used(self, action, ability_name=""):
        budget = getattr(self, "_action_budget", None)
        if not isinstance(budget, dict) or not budget.get("tick_started_at"):
            return
        budget[f"{action}_sent"] = True
        if action == "ability":
            budget["ability_name"] = str(ability_name or "")
    ...
    def _log_battle_tick_perf(self, tick_started_perf, current_time=None):
        ...
        interval = 2.0 if self._is_verbose_battle_debug() or tick_ms >= 120.0 else 10.0
        if current_time - getattr(self, "_last_battle_perf_log_at", 0.0) < interval:
            return
        print(
            "[BATTLE][PERF] "
    ...
    def _log_ability_skip(self, ability_name, reason, current_time=None):
        current_time = current_time if current_time is not None else time.time()
    ...
    def _use_ability(self, ability_name, key, cooldown, last_attr, reason="ready_and_engaged", before_dispatch=None, after_success=None):
        current_time = time.time()
        ...
        if not self._target_gate_allows("ability"):
            gate_reason = str(getattr(self, "_last_target_gate_block_reason", "") or "")
            if ability_name == "hypercharge":
                skip_reason = "no_fire_or_no_confirmed_engagement" if "no_fire" in gate_reason else "no_confirmed_engagement"
            else:
                skip_reason = "no_fire_or_target_not_confirmed" if "no_fire" in gate_reason else "target_not_confirmed"
        ...
        if self._action_used_this_tick("ability"):
            ...
            return False
        last_used = float(getattr(self, last_attr, 0.0) or 0.0)
        if cooldown > 0 and current_time - last_used < cooldown:
            self._log_ability_skip(ability_name, "cooldown", current_time)
            return False
        if before_dispatch:
            before_dispatch()
        dispatched = self.window_controller.press_key(key)
        if dispatched is False:
            print(f"[INPUT][ERROR] {ability_name} dispatch was not attempted")
            return False
        setattr(self, last_attr, current_time)
        self._mark_action_used("ability", ability_name)
        if after_success:
            after_success()
        print(f"[BATTLE] using {ability_name} reason={reason}")
        self._record_battle_input(ability_name, reason, current_time)
        return True

    def use_hypercharge(self, reason="ready_and_engaged"):
        return self._use_ability(
            "hypercharge",
            "H",
            float(getattr(self, "HYPERCHARGE_USE_COOLDOWN", 2.5) or 0.0),
            "time_since_hypercharge_used",
            reason=reason,
        )

    def use_gadget(self, reason="ready_and_engaged"):
        return self._use_ability(
            "gadget",
            "G",
            float(getattr(self, "GADGET_USE_COOLDOWN", 1.0) or 0.0),
            "time_since_gadget_used",
            reason=reason,
        )

    def use_super(self, reason="close_range_confirmed"):
        def before_dispatch():
            ...
            last_walls = getattr(self, "last_walls_data", None)
            self._pre_super_walls = list(last_walls) if last_walls else None
            self._force_wall_refresh = True
        ...
        return self._use_ability(
            "super",
            "E",
            float(getattr(self, "SUPER_USE_COOLDOWN", 1.0) or 0.0),
            "time_since_super_used",
            reason=reason,
    ...
    def _target_gate_allows(self, action):
        ...
        if not isinstance(gate, dict):
            ...
            self._target_gate = gate
        no_fire_reason = self._target_gate_no_fire_reason(action, gate)
        if no_fire_reason:
            self._last_target_gate_block_reason = no_fire_reason
            return False
        key = "allow_ability" if action == "ability" else "allow_attack"
        if not gate.get(key):
            self._last_target_gate_block_reason = str(gate.get("reason") or "stale_or_missing_target")
            return False
        confidence = float(gate.get("target_confidence", 0.0) or 0.0)
        min_confidence = 0.90 if action == "ability" else 0.80
        if confidence < min_confidence:
            self._last_target_gate_block_reason = "target_not_confirmed" if action == "ability" else "stale_or_missing_target"
            return False
        max_age = (
        ...
        allowed = target_age <= max_age and wall_age <= max_age
        if not allowed:
            self._last_target_gate_block_reason = "stale_or_missing_target"
        return allowed

    def _target_gate_no_fire_reason(self, action, gate=None):
        gate = gate if isinstance(gate, dict) else getattr(self, "_target_gate", {}) or {}
        current_time = time.time()
        if self._is_no_fire_spawn_window_active(current_time):
            return "no_fire_match_start" if action == "attack" else "no_fire_or_target_not_confirmed"
        source = str(
            getattr(self, "_authoritative_movement_source", "")
            or getattr(self, "_committed_analog_reason", "")
            or (getattr(self, "_battle_runtime", {}) or {}).get("active_strategy", "")
            or ""
        )
        if source in {"spawn_escape_no_vision", "brawlball_spawn_escape", "brawlball_opening"}:
            return "no_fire_match_start" if action == "attack" else "no_fire_or_target_not_confirmed"
        if source in {"brawlball_lane_push", "wall_escape", "corner_escape", "showdown_wall_escape", "showdown_border", "showdown_roam"}:
            confirmed = bool(gate.get("target_confirmed"))
            confidence = float(gate.get("target_confidence", 0.0) or 0.0)
            min_confidence = 0.90 if action == "ability" else 0.80
            if not confirmed or confidence < min_confidence:
                return "no_confirmed_target_or_no_fire_state" if action == "attack" else "no_fire_or_target_not_confirmed"
        return ""

    def _log_target_gate_skip(self, action, reason, current_time=None):
        current_time = current_time if current_time is not None else time.time()
        if action == "attack" and reason in {"stale_or_missing_target", "no_fire_match_start"}:
            reason = "no_confirmed_target_or_no_fire_state"
        key = f"{action}:{reason}"
        if not isinstance(getattr(self, "_ability_skip_log_at", None), dict):
            self._ability_skip_log_at = {}
        interval = 1.2 if self._is_verbose_battle_debug() else 4.0
        if current_time - self._ability_skip_log_at.get(key, 0.0) >= interval:
    ...
    def unstuck_movement_if_needed(self, movement, current_time=None):
        ...
        if is_oscillating:
            ...
            # No enemies â€” still trigger anti-stuck if stuck for >4s
            ...
            if idle_dur < 4.0:
                ...
                print(f"[STUCK] No enemies, idle {idle_dur:.1f}s â€” forcing unstuck")
...
class Play(Movement):
    ...
    def __init__(self, main_info_model, tile_detector_model, window_controller):
        ...
        self.scene_data = []
        self.should_detect_walls = self.is_showdown or self.game_mode_name in [
            "brawlball",
            "brawl_ball",
            "brawll ball",
            "knockout",
    ...
    def _note_battle_tick(self, current_time, brawler):
        ...
        if self._is_verbose_battle_debug() and current_time - self._last_battle_tick_log_at >= 2.0:
            ...
            print(
                f"[BATTLE] tick brawler={brawler_key or 'unknown'} logic={logic_name} "
                f"role={role} desired_range={desired_range} strategy={strategy}"
            )
    ...
    def _set_battle_skip_reason(self, reason, current_time=None, fallback=False):
        ...
        interval = 1.5 if self._is_verbose_battle_debug() else 4.0
        if current_time - last_log_at >= interval:
    ...
    def _record_movement_result(self, result, detail, current_time=None):
        ...
        if isinstance(result, dict):
            ...
            self._reset_movement_watchdog()
        self._mark_action_used("movement")
    ...
    def reset_match_control_state(self, current_time=None, brawler=None):
        ...
        self._brawl_ball_spawn_escape_active = False
        self._brawl_ball_spawn_escape_complete = False
        self._brawl_ball_spawn_escape_started_at = current_time
        self._brawl_ball_spawn_escape_last_nudge_at = 0.0
        self._last_brawl_ball_spawn_escape_log_at = 0.0
        self._last_brawl_ball_opening_log_at = 0.0
        self._last_brawl_ball_opening_override_log_at = 0.0
        ...
        self._authoritative_movement_source = ""
        self._authoritative_movement_at = 0.0
        self._reset_wall_escape_state(current_time)
        self._planned_analog_reason = None
        self._committed_analog_reason = None
        self._committed_analog_until = 0.0
        self._movement_anchor_angle = None
        self._movement_anchor_angle_pos = None
        if isinstance(getattr(self, "fix_angle_state", None), dict):
            self.fix_angle_state["toggled"] = False
        if isinstance(getattr(self, "fix_movement_keys", None), dict):
            self.fix_movement_keys["toggled"] = False
        if isinstance(getattr(self, "escape_state", None), dict):
            self.escape_state["phase"] = None
        if isinstance(getattr(self, "wall_stuck_state", None), dict):
    ...
    def _is_inside_own_brawl_ball_spawn(self, player_pos):
        if not player_pos:
            return True
    ...
    def _clear_spawn_stale_movement(self, current_time=None):
        ...
        self._planned_analog_reason = None
        self._set_authoritative_movement_angle(270.0, "spawn_escape_no_vision", current_time)

    def _set_authoritative_movement_angle(self, angle, source, current_time=None):
        if not isinstance(angle, (float, int)):
            return None
        current_time = current_time if current_time is not None else time.time()
        self._authoritative_movement_angle = float(angle) % 360.0
        self._authoritative_movement_source = str(source or "")
        self._authoritative_movement_at = current_time
        return self._authoritative_movement_angle

    def _authoritative_watchdog_angle(self, data=None, current_time=None):
        current_time = current_time if current_time is not None else time.time()
        status = self._brawlball_no_vision_spawn_status(data, current_time)
        if status:
            angle = float(status.get("angle", 270.0) or 270.0)
            if self._angle_difference(angle, 270.0) > 35.0:
                print(f"[INPUT] movement watchdog rejected stale angle={angle:.0f} source=spawn_escape")
                angle = 270.0
            return angle, "spawn_escape_no_vision"
        ...
        if bool(getattr(self, "is_showdown_mode", False)):
            ...
            if player_pos is not None:
                wall_context = (data or {}).get("wall") or {}
                players = (data or {}).get("player") or []
        ...
        if self._is_brawl_ball_mode(getattr(self, "selected_gamemode", "")):
            stale = getattr(self, "last_movement", None)
            if isinstance(stale, (float, int)) and self._angle_difference(float(stale), 270.0) > 45.0:
                print(f"[INPUT] movement watchdog rejected stale angle={float(stale):.0f} source={source or 'last_movement'}")
            player_pos = self._player_position_from_data(data)
            lane_angle = self._get_brawl_ball_lane_push_angle(player_pos, current_time, (data or {}).get("wall") or [])
            return float(lane_angle), "brawlball_lane_push"

        if isinstance(angle, (float, int)):
            return float(angle) % 360.0, source or "last_movement"
        stale = getattr(self, "last_movement", None)
    ...
    def _reset_wall_escape_state(self, current_time=None, log_complete=False):
        if log_complete and getattr(self, "_wall_escape_state", {}).get("active"):
            print("[BATTLE] wall escape complete")
        self._wall_escape_state = {
            "active": False,
            "source": "",
            "escape_source": "",
            "angle": None,
            "started_at": 0.0,
            "last_seen_at": current_time or 0.0,
            "count": 0,
            "nudge_until": 0.0,
            "return_until": 0.0,
            "nudge_angle": None,
    ...
    def _wall_blocked_escape_angle(self, angle, source, player_pos, wall_context, current_time=None):
        if not isinstance(angle, (float, int)) or player_pos is None or wall_context is None:
            return None
        current_time = current_time if current_time is not None else time.time()
        ...
        try:
            blocked = bool(
                wall_context.get("rectangles")
                and self._is_path_blocked_angle(player_pos, float(angle), wall_context)
            )
        except Exception:
            blocked = False
        if not blocked:
            if state.get("active"):
                self._reset_wall_escape_state(current_time, log_complete=True)
            return None

        same_block = (
            state.get("angle") is not None
            and self._angle_difference(float(state.get("angle")), float(angle)) <= 24.0
            and str(state.get("source") or "") == source
        )
        if not same_block:
            state = {
                **state,
        ...
        if bool(getattr(self, "is_showdown_mode", False)) and not is_brawl_ball:
            ...
            if callable(finder):
                nudge_angle = finder(player_pos, retreat_angle, wall_context)
                base_angle = finder(player_pos, arc_angle, wall_context)
            else:
                nudge_angle = retreat_angle
                base_angle = arc_angle
        ...
        state.update({
            ...
            + float(getattr(self, "_wall_escape_return_seconds", 0.65) or 0.65),
            "started_at": current_time,
            "count": 0,
            "return_logged": False,
        })
        self._wall_escape_state = state
        print(f"[BATTLE] wall-blocked detected angle={float(angle):.0f} source={source} duration={duration:.2f}")
        if source == "spawn_escape_no_vision":
    ...
    def _brawlball_no_vision_spawn_status(self, data=None, current_time=None):
        ...
        uncertain_seconds = max(
            min_seconds,
            float(getattr(self, "_brawl_ball_spawn_escape_uncertain_seconds", 10.0) or 10.0),
        )
        extended_seconds = max(
            uncertain_seconds + 1.0,
            float(getattr(self, "_brawl_ball_spawn_escape_extended_seconds", 14.0) or 14.0),
        )
        ...
        if reliable_self and not inside_spawn and elapsed >= min_seconds:
            ...
            self._brawl_ball_spawn_escape_active = False
            return None
        if not reliable_self and elapsed >= extended_seconds:
            if current_time - getattr(self, "_last_brawl_ball_spawn_escape_log_at", 0.0) >= 1.0:
                print("[BATTLE] spawn escape fallback to lane push reason=extended_unknown")
                self._last_brawl_ball_spawn_escape_log_at = current_time
            self._brawl_ball_spawn_escape_complete = True
            self._brawl_ball_spawn_escape_active = False
            self._set_authoritative_movement_angle(270.0, "brawlball_lane_push", current_time)
            return None
        ...
        if elapsed > uncertain_seconds and not reliable_self:
            reason = "self_unknown_timeout"
            if current_time - getattr(self, "_last_brawl_ball_spawn_escape_log_at", 0.0) >= 1.0:
                print("[BATTLE] spawn escape uncertain reason=self_unknown_timeout")
                self._last_brawl_ball_spawn_escape_log_at = current_time
    ...
    def _force_brawlball_no_vision_spawn_escape(self, data=None, current_time=None, source="spawn_escape_no_vision"):
        ...
        self._set_authoritative_movement_angle(angle, "spawn_escape_no_vision", current_time)
    ...
    def _get_brawl_ball_spawn_escape_angle(self, player_pos=None, current_time=None, candidate_angle=None, reason="inside_own_goal"):
        ...
        max_seconds = max(
            float(getattr(self, "_brawl_ball_spawn_escape_seconds", 8.0) or 8.0),
            float(getattr(self, "_brawl_ball_opening_seconds", 6.0) or 6.0),
        )
        min_seconds = max(0.0, float(getattr(self, "_brawl_ball_spawn_escape_min_seconds", 5.8) or 5.8))
        inside_spawn = self._is_inside_own_brawl_ball_spawn(player_pos)
        missing_self = player_pos is None
        uncertain_seconds = max(
            min_seconds,
            float(getattr(self, "_brawl_ball_spawn_escape_uncertain_seconds", 10.0) or 10.0),
        )
        ...
        if elapsed > max_seconds and not inside_spawn and not missing_self:
            ...
            print("[BATTLE] spawn-box escape complete reason=reliable_outside_spawn")
            return None
        if elapsed >= extended_seconds and missing_self:
            if current_time - getattr(self, "_last_brawl_ball_spawn_escape_log_at", 0.0) >= 1.0:
                print("[BATTLE] spawn escape fallback to lane push reason=extended_unknown")
                self._last_brawl_ball_spawn_escape_log_at = current_time
            self._brawl_ball_spawn_escape_complete = True
            ...
            self._set_authoritative_movement_angle(270.0, "brawlball_lane_push", current_time)
            return None
        if elapsed >= uncertain_seconds and missing_self:
            if current_time - getattr(self, "_last_brawl_ball_spawn_escape_log_at", 0.0) >= 1.0:
                print("[BATTLE] spawn escape uncertain reason=self_unknown_timeout")
                self._last_brawl_ball_spawn_escape_log_at = current_time
        ...
        if not getattr(self, "_brawl_ball_spawn_escape_active", False):
            self._brawl_ball_spawn_escape_started_at = current_time
        self._brawl_ball_spawn_escape_active = True

        if candidate_angle is not None:
            try:
                candidate_delta = self._angle_difference(float(candidate_angle), 270.0)
            except (TypeError, ValueError):
                candidate_delta = 180.0
            if candidate_delta > 28.0 and current_time - getattr(self, "_last_brawl_ball_spawn_escape_log_at", 0.0) >= 0.8:
                print(f"[BATTLE] spawn-box escape locked; rejecting angle={float(candidate_angle):.0f} reason=still_inside_spawn")
                self._last_brawl_ball_spawn_escape_log_at = current_time
        ...
        if (
            ...
            if player_pos:
                try:
                    ...
                    pass
            angle = 285.0 if x_pos < width * 0.5 else 255.0
            self._brawl_ball_spawn_escape_last_nudge_at = current_time
            print(f"[BATTLE] spawn-box escape nudge angle={angle:.0f} reason=blocked")
            if elapsed >= nudge_after + nudge_interval:
                print("[BATTLE] spawn-loop detected; forcing escape route")
        elif current_time - getattr(self, "_last_brawl_ball_spawn_escape_log_at", 0.0) >= 1.0:
            print(f"[BATTLE] spawn-box escape active angle=270 reason={reason}")
            self._last_brawl_ball_spawn_escape_log_at = current_time

        self._brawl_ball_opening_angle = float(angle)
        self._brawl_ball_opening_angle_until = current_time + float(
            getattr(self, "_brawl_ball_opening_hold_seconds", 1.4) or 1.4
        )
        self._set_battle_strategy("brawlball_spawn_escape")
        return float(angle)
    ...
    def _get_brawl_ball_opening_angle(self, player_pos=None, current_time=None):
        ...
        current_time = current_time if current_time is not None else time.time()
        elapsed = self._brawl_ball_opening_elapsed(current_time)
        spawn_escape_angle = self._get_brawl_ball_spawn_escape_angle(player_pos, current_time)
        if spawn_escape_angle is not None:
            return spawn_escape_angle
        if elapsed is None or elapsed > float(getattr(self, "_brawl_ball_opening_seconds", 0.0) or 0.0):
            if getattr(self, "_brawl_ball_opening_angle", None) is not None:
                self._brawl_ball_opening_angle = None
                self._set_battle_strategy("brawlball_objective")
                print("[BATTLE] brawlball opening complete; switching to strategy=brawlball_objective")
            return None
        width = max(1.0, float(getattr(self.window_controller, "width", brawl_stars_width) or brawl_stars_width))
        height = max(1.0, float(getattr(self.window_controller, "height", brawl_stars_height) or brawl_stars_height))
        lock_seconds = max(0.0, float(getattr(self, "_brawl_ball_opening_lock_seconds", 0.0) or 0.0))
        if elapsed <= lock_seconds:
            angle = 270.0
            self._brawl_ball_opening_angle_until = current_time + float(
                getattr(self, "_brawl_ball_opening_hold_seconds", 1.4) or 1.4
            )
            remaining = max(0.0, lock_seconds - elapsed)
            if current_time - getattr(self, "_last_brawl_ball_opening_log_at", 0.0) >= 1.0:
                print(f"[BATTLE] brawlball opening locked angle=270 remaining={remaining:.1f}s")
                print("[BATTLE] spawn escape route active")
                self._last_brawl_ball_opening_log_at = current_time
            self._brawl_ball_opening_angle = angle
    ...
    def _brawl_ball_opening_lock_active(self, current_time=None):
        if not self._is_brawl_ball_mode():
            return False
        current_time = current_time if current_time is not None else time.time()
        elapsed = self._brawl_ball_opening_elapsed(current_time)
        if elapsed is None:
            return False
        return elapsed <= max(0.0, float(getattr(self, "_brawl_ball_opening_lock_seconds", 0.0) or 0.0))

    def _log_brawl_ball_opening_override(self, accepted, reason, current_time=None):
        current_time = current_time if current_time is not None else time.time()
        if current_time - getattr(self, "_last_brawl_ball_opening_override_log_at", 0.0) < 0.75:
            return
        if accepted:
            print(f"[BATTLE] brawlball opening override accepted reason={reason}")
        else:
    ...
    def _stabilize_analog_angle(self, angle, current_time):
        ...
        self._planned_analog_reason = None
        hold_times = getattr(self, "_analog_goal_hold_times", {}) or {}
        priorities = getattr(self, "_analog_goal_priorities", {}) or {}
        if reason == "spawn_escape_no_vision":
            if self._angle_difference(angle, 270.0) > 35.0:
                print(f"[BATTLE] spawn-box no-vision rejected movement source=smoothing angle={angle:.0f}")
                angle = 270.0
            self._clear_spawn_stale_movement(current_time)
            self._committed_analog_reason = reason
            self._committed_analog_until = current_time + hold_times.get(reason, 1.4)
            return angle
        if reason == "brawlball_lane_push":
            previous_reason = str(getattr(self, "_committed_analog_reason", "") or "")
            if previous_reason == "spawn_escape_no_vision" and current_time < float(getattr(self, "_committed_analog_until", 0.0) or 0.0):
                print(f"[BATTLE] spawn-box no-vision rejected movement source=lane_push angle={angle:.0f}")
                return 270.0

        if self.last_movement is None or not isinstance(self.last_movement, (float, int)):
            self._committed_analog_reason = reason
            self._committed_analog_until = current_time + hold_times.get(reason, 0.16)
            return angle

        angle_delta = self._angle_difference(angle, self.last_movement)
        current_reason = self._committed_analog_reason or reason
        current_priority = priorities.get(current_reason, 0)
        next_priority = priorities.get(reason, 0)
        ...
        if current_time < self._committed_analog_until:
            if next_priority < current_priority and angle_delta < self._analog_turn_emergency_threshold:
                if current_time - getattr(self, "_last_angle_smoothing_log_at", 0.0) >= 1.0:
                    print(f"[BATTLE] movement smoothing kept angle={float(self.last_movement):.0f}")
                    self._last_angle_smoothing_log_at = current_time
                return float(self.last_movement)
            if next_priority == current_priority and angle_delta < self._analog_turn_emergency_threshold:
                if current_time - getattr(self, "_last_angle_smoothing_log_at", 0.0) >= 1.0:
                    print(
                        f"[BATTLE] movement jitter suppressed old={float(self.last_movement):.0f} "
                        f"new={float(angle):.0f}"
                    )
                    self._last_angle_smoothing_log_at = current_time
                return float(self.last_movement)
        ...
        self._committed_analog_reason = reason
        self._committed_analog_until = current_time + hold_times.get(reason, 0.16)
        if angle_delta >= self._analog_turn_threshold and current_time - getattr(self, "_last_angle_smoothing_log_at", 0.0) >= 1.0:
            print(f"[BATTLE] movement smoothing accepted angle={float(angle):.0f}")
            self._last_angle_smoothing_log_at = current_time
        return angle
    ...
    def _get_corner_escape_angle(self, player_pos, current_time=None, no_target=True):
        ...
        if current_time - getattr(self, "_last_corner_escape_log_at", 0.0) >= 1.0:
            ...
            print(f"[BATTLE] corner escape active angle={angle:.0f} reason=bottom_corner_no_target")
            self._last_corner_escape_log_at = current_time
        self._set_battle_strategy("corner_escape")
        self._set_authoritative_movement_angle(angle, "corner_escape", current_time)
    ...
    def _get_brawl_ball_lane_push_angle(self, player_pos, current_time=None, wall_context=None):
        ...
        if current_time - getattr(self, "_last_brawl_ball_lane_log_at", 0.0) >= 1.2:
            ...
            self._last_brawl_ball_lane_log_at = current_time
        self._set_battle_strategy("brawlball_objective")
        self._set_authoritative_movement_angle(desired, "brawlball_lane_push", current_time)
        ...
        try:
            ...
            if self._uses_analog_movement() and hasattr(self.window_controller, "move_joystick_angle"):
                if self._is_brawl_ball_mode():
                    ...
                    self.last_movement_time = current_time
                elif bool(getattr(self, "is_showdown_mode", False)):
                    angle, _source = self._authoritative_watchdog_angle(None, current_time)
                else:
    ...
    def _get_showdown_fog_escape_move(self, player_data, wall_context):
        if not bool(getattr(self, "is_showdown_mode", False)):
            return None
        if not (self._is_storm_flee_delay_over() and (self._storm_radius < 5000 or self._gas_active)):
            return None
        ...
        if not (self._in_storm or dist_to_center > self._storm_radius * 0.85):
            return None
        desired_angle = self.angle_from_direction(
            self._storm_center[0] - player_position[0],
            self._storm_center[1] - player_position[1],
        )
        return self._showdown_best_angle(player_position, desired_angle, wall_context)
    ...
    def no_enemy_movement(self, player_data, walls, playstyle="fighter", teammates=None):
        ...
        if not isinstance(getattr(self, "time_since_detections", None), dict):
            self.time_since_detections = {"enemy": now, "player": now}
        if not isinstance(getattr(self, "_visited_zones", None), list):
            self._visited_zones = []
        if not hasattr(self, "_storm_center"):
            self._storm_center = (960, 540)
        if not hasattr(self, "_storm_radius"):
            self._storm_radius = 9999
        if not hasattr(self, "_gas_active"):
            self._gas_active = False
        if not hasattr(self, "_in_storm"):
            self._in_storm = False
        if not hasattr(self, "_match_start_time"):
            self._match_start_time = now
        if not hasattr(self, "_storm_flee_delay_sec"):
            self._storm_flee_delay_sec = 30.0
        ...
        if bool(getattr(self, "is_showdown_mode", False)):
            if teammates:
                self._teammate_positions = self._showdown_teammate_centers(teammates)
            fog_escape = self._get_showdown_fog_escape_move(player_data, walls)
            if fog_escape is not None:
                self.last_decision_reason = "SHOWDOWN: gas/fog escape"
                self._set_battle_strategy("showdown_survive")
                return self._plan_analog_reason(fog_escape, "fog_escape")
            if self._showdown_border_enabled():
                border_move = self._get_showdown_border_move(player_position, walls)
                self.last_decision_reason = "SHOWDOWN: safe border"
            ...
            if support_move is not None:
                strategy = {
                    ...
                    "showdown_border": "showdown_border",
                    "fog_escape": "showdown_survive",
                }.get(support_reason, f"showdown_{support_reason or 'explore'}")
                self.last_decision_reason = f"SHOWDOWN: {support_reason or 'explore'}"
                self._set_battle_strategy(strategy)
                return self._plan_analog_reason(support_move, support_reason or "showdown_roam")
    ...
    def _showdown_teammate_centers(self, teammates=None):
        centers = []
        for item in list(teammates or []) + list(getattr(self, "_teammate_positions", []) or []):
            try:
                if len(item) >= 4:
                    centers.append(((float(item[0]) + float(item[2])) / 2.0, (float(item[1]) + float(item[3])) / 2.0))
                elif len(item) >= 2:
        ...
        for center in centers:
            if not any(self.get_distance(center, other) < 8.0 for other in unique):
                unique.append(center)
        return unique

    def _showdown_best_angle(self, player_pos, desired_angle, wall_context):
        finder = getattr(self, "_find_best_angle", None)
        if callable(finder):
            return finder(player_pos, desired_angle, wall_context)
    ...
    def _get_showdown_regroup_target(self, player_pos):
        ...
        current_time = time.time()
        if locked and current_time < getattr(self, "_showdown_teammate_lock_until", 0.0):
            return locked, locked_distance
        if locked and nearest_distance >= locked_distance * (1.0 - getattr(self, "_showdown_teammate_hysteresis", 0.2)):
            self._showdown_locked_teammate = locked
            self._showdown_locked_teammate_distance = locked_distance
            return locked, locked_distance
        self._showdown_locked_teammate = nearest
        self._showdown_locked_teammate_distance = nearest_distance
        self._showdown_teammate_lock_until = current_time + getattr(self, "_showdown_teammate_lock_duration", 0.55)
        return nearest, nearest_distance

    def _pick_showdown_orbit_side(self):
        current_time = time.time()
        if current_time < getattr(self, "_showdown_orbit_until", 0.0) and getattr(self, "_showdown_orbit_side", 1) in (-1, 1):
            return self._showdown_orbit_side
        self._showdown_orbit_side = -1 if getattr(self, "_showdown_orbit_side", 1) == 1 else 1
        self._showdown_orbit_until = current_time + getattr(self, "_showdown_orbit_switch_interval", 1.8)
        return self._showdown_orbit_side
    ...
    def _get_showdown_follow_move(self, player_pos, wall_context, allow_spacing=True, allow_orbit=False):
        ...
        spacing_distance = max(88.0, min(132.0, getattr(self, "_showdown_teammate_spacing_distance", 180.0)))
        orbit_distance = max(spacing_distance + 40.0, min(260.0, getattr(self, "_showdown_teammate_orbit_distance", 520.0)))
        if getattr(self, "_showdown_trio_grouping_enabled", True) and allow_spacing:
            if teammate_distance < spacing_distance:
                return self._showdown_best_angle(player_pos, self.angle_opposite(teammate_angle), wall_context), teammate_distance, "team_space"
            if allow_orbit and teammate_distance <= orbit_distance:
                desired_angle = (teammate_angle + (90.0 * self._pick_showdown_orbit_side())) % 360.0
                return self._showdown_best_angle(player_pos, desired_angle, wall_context), teammate_distance, "team_orbit"
        return self._showdown_best_angle(player_pos, teammate_angle, wall_context), teammate_distance, "team_follow"
    ...
    def _get_showdown_border_move(self, player_pos, wall_context):
        ...
        if now < getattr(self, "_showdown_border_angle_until", 0.0):
            ...
            self._showdown_border_angle_until = now + max(1.0, float(getattr(self, "_showdown_border_hold_seconds", 2.4) or 2.4))
        desired_angle = self.angle_from_direction(side_x - x_pos, target_y - y_pos)
        return self._showdown_best_angle(player_pos, desired_angle, wall_context)
    ...
    def _get_showdown_roam_move(self, player_pos, wall_context):
        """Stable Showdown exploration helper used by fallback logic and tests."""
        now = time.time()
        width = max(1.0, float(getattr(self.window_controller, "width", brawl_stars_width) or brawl_stars_width))
        height = max(1.0, float(getattr(self.window_controller, "height", brawl_stars_height) or brawl_stars_height))
        targets = [
        ...
        if now >= getattr(self, "_showdown_roam_angle_until", 0.0):
            idx = (int(getattr(self, "_showdown_roam_target_index", 0) or 0) + 1) % len(targets)
            self._showdown_roam_target_index = idx
            self._showdown_roam_angle_until = now + max(
                1.0,
                float(getattr(self, "_showdown_roam_hold_seconds", 2.2) or 2.2),
            )
        idx = int(getattr(self, "_showdown_roam_target_index", 0) or 0) % len(targets)
        target = targets[idx]
        if self.get_distance(player_pos, target) < 130.0:
            idx = (idx + 1) % len(targets)
            self._showdown_roam_target_index = idx
            target = targets[idx]
            self._showdown_roam_angle_until = now + max(1.0, float(getattr(self, "_showdown_roam_hold_seconds", 2.2) or 2.2))
    ...
    def get_main_data(self, frame, runtime_state="", current_time=None):
        ...
        _hud_max_bbox = int(_fw * 0.08)       # ~128px at 1600w â€” real chars are bigger
    ...
    def loop(self, brawler, data, current_time):
        ...
        if not data.get('enemy'):
            if self._is_brawl_ball_mode(getattr(self, "selected_gamemode", "")):
                self._set_battle_skip_reason("brawlball lane push; no confirmed target", current_time=current_time)
            else:
        ...
        player_pos = self._player_position_from_data(data)
        if player_pos is None and (data.get("player") or []):
            player_pos = self.get_player_pos(data["player"][0])
        wall_context = data.get("wall") or []
        ...
        watchdog_due = (
            ...
            and movement_silence > max(
                self.minimum_movement_delay,
                float(getattr(self, "_movement_watchdog_seconds", 0.65) or 0.65),
            )
        )
        if movement_silence > self.minimum_movement_delay or watchdog_due:
            ...
            if isinstance(movement, (float, int)):
                ...
                movement_reason = str(getattr(self, "_committed_analog_reason", "") or getattr(self, "_authoritative_movement_source", "") or "")
                if (
                    self._is_brawl_ball_mode(getattr(self, "selected_gamemode", ""))
                    and not (data.get("enemy") or [])
                    and movement_reason not in {"spawn_escape_no_vision", "brawlball_lane_push", "wall_escape", "corner_escape", "brawlball_opening"}
                ):
                    print(f"[BATTLE] brawlball no-target rejected generic_safe angle={float(movement):.0f} reason=lane_lock")
                    movement = self._get_brawl_ball_lane_push_angle(player_pos, current_time, wall_context)
                    movement = self._stabilize_analog_angle(movement, current_time)
                    movement_reason = str(getattr(self, "_committed_analog_reason", "") or "brawlball_lane_push")
            ...
            if not movement and str(getattr(self, "_runtime_state", "") or "") == "match":
                movement = "W" if self.game_mode == 3 else "D"
                self._set_battle_skip_reason(
                    "movement decision empty; using safe generic movement",
                    current_time=current_time,
                    fallback=True,
                )
            self._battle_runtime["last_movement"] = str(movement)
            if not (data.get("enemy") or []):
                movement_reason = str(getattr(self, "_committed_analog_reason", "") or "")
                if movement_reason == "spawn_escape_no_vision":
                    self._set_battle_skip_reason(
                        f"spawn escape no vision; moving upfield ({movement})",
                        current_time=current_time,
                    )
                elif movement_reason == "brawlball_lane_push":
                    self._set_battle_skip_reason(
                        f"brawlball lane push; no confirmed target ({movement})",
                        current_time=current_time,
                    )
                elif movement_reason == "wall_escape":
                    self._set_battle_skip_reason(
                ...
                elif movement_reason == "brawlball_opening":
                    self._set_battle_skip_reason(
                        f"brawlball opening route; no confirmed target ({movement})",
                        current_time=current_time,
                    )
                else:
                    self._set_battle_skip_reason(
                        f"no target found; moving safely ({movement})",
                        current_time=current_time,
                    )
            else:
    ...
    def _color_based_wall_detection(self, frame):
        ...
        Targets teal/cyan stone walls (Hâ‰ˆ75-110) that the ONNX tile detector
    ...
    def combine_walls_from_history(self):
        ...
        deduplication fails. Instead we:
          ...
          2. Cluster overlapping boxes (IoU â‰¥ 0.3) together.
          3. Average the coordinates within each cluster for a stable box.
          4. Require a wall to be seen in â‰¥2 frames to suppress hallucinations.
        """
        ...
        combined_walls = []
        min_frames = 1  # Accept walls seen in just 1 frame (was min(2,...) â€” too strict)
        ...
        for cluster in clusters:
            ...
            if unique_frames < required_frames:
                continue
            # Require at least one hit in latest 3 scans to avoid stale ghosts (was 2 â€” too strict).
    ...
    def get_movement(self, player_data, enemy_data, walls, brawler, teammates=None):
        ...
        if enemy_coords is None or enemy_distance is None:
            if self._is_brawl_ball_mode():
                self._battle_runtime["last_attack_decision"] = "target scan returned no confident target"
                self._set_battle_strategy("brawlball_objective")
                return self._get_brawl_ball_lane_push_angle(player_pos, now, walls)
            return self.no_enemy_movement(player_data, walls, playstyle, teammates=teammates)
        if self._brawl_ball_opening_lock_active(now):
            try:
                target_hittable = self.is_enemy_hittable(player_pos, enemy_coords, walls, "attack")
            except Exception:
                target_hittable = False
            enemy_angle = self.angle_from_direction(enemy_coords[0] - player_pos[0], enemy_coords[1] - player_pos[1])
            spawn_escape_angle = self._get_brawl_ball_spawn_escape_angle(
                player_pos,
                now,
                candidate_angle=enemy_angle,
                reason="still_inside_spawn",
            )
            high_confidence_override = (
                bool(target_hittable)
                and enemy_distance <= min(220.0, attack_range * 0.65, safe_range * 1.05)
            )
            if spawn_escape_angle is not None and not high_confidence_override:
                self._log_brawl_ball_opening_override(False, "still_inside_spawn", now)
                opening_move = self._get_brawl_ball_opening_move(player_pos, now)
                if opening_move:
                    self.last_decision_reason = "BRAWL BALL SPAWN ESCAPE LOCK"
                    self._set_battle_strategy("brawlball_spawn_escape")
                    return opening_move
            elif not high_confidence_override:
                self._log_brawl_ball_opening_override(False, "low_confidence/noisy", now)
                opening_move = self._get_brawl_ball_opening_move(player_pos, now)
                if opening_move:
                    self.last_decision_reason = "BRAWL BALL OPENING LOCK"
                    self._set_battle_strategy("brawlball_opening")
                    return opening_move
            else:
        ...
        # Low-HP brawlers (â‰¤2600) retreat earlier, tanky brawlers (â‰¥4500) retreat later
        hp_retreat_adj = 1.0
        ...
        if low_hp:
            ...
            # Rush more aggressively if we can kill in â‰¤2 attacks
            ...
            if has_cover:
                ...
                for wall in (walls or []):
                    ...
                    # Prefer walls that are roughly between us and enemy (Â±90Â°)
        ...
        if burst_active:
            ...
            # High-damage brawlers (â‰¥2000 dmg) space shots out to aim carefully
            # Low-damage brawlers (<1000 dmg) spam faster for DPS
            # Multi-projectile brawlers (â‰¥3 projectiles) fire slightly faster (wider spread = forgiving)
    ...
    def main(self, frame, brawler):
        ...
        self._begin_action_tick(current_time)
        ...
        if not data:
            ...
            if current_time - self.time_since_last_proceeding > self.no_detection_proceed_delay:
                ...
                likely_post_match_reward = (
                    time_since_player >= 2.5
                    and raw_supporting_entities <= 0
                    and (current_time - self.time_since_detections.get('enemy', 0)) >= 1.5
                )
                allow_reward_ocr = (
                    current_state == "match"
        ...
        try:
            ...
            if enemies and 'player' in data and data['player']:
                ...
                # time window (0.3â€“3 s) already filters random de-spawns.
