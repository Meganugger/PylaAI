...
_CONFIG_DEFAULTS = {
    ...
    "cfg/bot_config.toml": {
        ...
        "post_match_action_guard_reset_seconds": 4.0,
        ...
        "showdown_roam_hold_seconds": 1.25,
...
def _developer_api_post(session, endpoint, payload, timeout):
    response = session.post(
        ...
        json=payload,
        timeout=timeout,
        headers={"Accept": "application/json, text/plain, */*", "Origin": "https://developer.brawlstars.com", "Referer": "https://developer.brawlstars.com/"},
    )
...
def _general_brawlstars_api_config():
    general_config = load_toml_as_dict("cfg/general_config.toml")
    return {"api_token": str(general_config.get("brawlstars_api_key", "") or "").strip(), "player_tag": get_config_player_tag({}), "timeout_seconds": 15, "auto_refresh_token": "no"}
...
def refresh_brawl_stars_api_token_if_enabled(config, file_path="cfg/brawl_stars_api.toml", force=False):
    ...
    public_ip = get_public_ip(str(config.get("public_ip_service", "https://api.ipify.org") or "https://api.ipify.org").strip())
    ...
    created = _developer_api_post(session, "apikey/create", {"name": key_name, "description": str(config.get("key_description", "Auto-generated by PylaAI for the current public IP.")), "cidrRanges": [public_ip], "scopes": scopes}, timeout)
...
def fetch_brawl_stars_player(api_token, player_tag, timeout=15):
    ...
    response = requests.get(f"https://api.brawlstars.com/v1/players/{cleaned_tag.replace('#', '%23')}", headers={"Authorization": f"Bearer {api_token}"}, timeout=timeout)
    if response.status_code == 200:
        return response.json()
    try:
        error_payload = response.json()
