...
def refresh_brawl_stars_api_token_if_enabled(config, file_path="cfg/brawl_stars_api.toml", force=False):
    ...
    created = _developer_api_post(
        session,
        "apikey/create",
        {"name": key_name, "description": description, "cidrRanges": [public_ip], "scopes": scopes},
        timeout,
    )
...
def load_brawl_stars_api_config(file_path="cfg/brawl_stars_api.toml", force_refresh=False):
    ...
    try:
        ...
        for key in ("developer_email", "developer_password", "public_ip_service", "key_name_prefix", "key_description", "last_public_ip"):
